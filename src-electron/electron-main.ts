...
ipcMain.handle('raw', async (_ipcMainInvokeEvent: Electron.IpcMainInvokeEvent, query: string) => {
  return await dbIndex.raw(query);
});
...
ipcMain.handle('createBulk', async (_ipcMainInvokeEvent: Electron.IpcMainInvokeEvent, data: object[]) => {
  return await dbIndex.createBulk(data);
});
...
ipcMain.handle('updateBulk', async (_ipcMainInvokeEvent: Electron.IpcMainInvokeEvent, data: object[]) => {
  return await dbIndex.updateBulk(data);
});
...
ipcMain.handle('deleteBulk', async (_ipcMainInvokeEvent: Electron.IpcMainInvokeEvent, data: object[]) => {
  return await dbIndex.deleteBulk(data);
});
...
ipcMain.handle('count', async (_ipcMainInvokeEvent: Electron.IpcMainInvokeEvent, table: string, where = {}) => {
  return await dbIndex.count(table, where);
});
